...
// Ordinary network gaps must be caught up smoothly.
// A coordinate warp is reserved for a large jump in the authoritative target itself (teleport, reconnect or another explicit discontinuity), never merely because the local replica fell behind.
...
// The SDK facade therefore carries the last commanded assist velocity across frames instead of rebuilding the ramp from GET_ENTITY_VELOCITY.
// This keeps acceleration smooth without getting stuck at its first tiny increment.
...
inline constexpr float kRemoteMotionMaximumVerticalAccelerationMetersPerSecond2 =
    18.0F;
// Native locomotion owns small tracking errors.
// Physical velocity joins only after a meaningful drift and remains active until the replica is close, preventing sub-metre start/stop bursts around the marker.
...
// While a replica is behind, the root must face its authoritative destination rather than the independently replicated aim/character heading.
// This prevents an aiming strafe task from pulling the ped away from the marker after a stop-and-reverse transition.
...
// NativeDB documents 1.15 as this native's supported ceiling.
// V12.2 requested 1.75, but the game did not visibly accelerate the animation: the engine clamps or ignores that unsupported range.
// Catch-up above sprint speed must therefore come from route recovery, never an invalid move-rate value.
...
// Network speed samples can hover around a gait threshold for several snapshots.
// Holding the selected gait a little longer prevents the native task graph from repeatedly restarting walk/run animations.
inline constexpr std::uint32_t kRemoteMotionMinimumGaitDwellMs = 350U;
// Direct velocity is excellent in open terrain but fights collision when the target is behind a wall.
// A short progress probe promotes the puppet to an RDR2 navmesh task only after sustained evidence of being blocked.
...
// A ped can pass beside a recorded point without entering the narrow reached radius.
// Search only a bounded prefix and discard points preceding the closest route anchor, so recovery can never walk backwards through an ever-growing queue.
...
// Keeping minutes of history made a released puppet visit obsolete points across Valentine before it could chase the current authoritative target.
...
inline constexpr float kRemoteNavigationSafeRecoveryMinimumDistanceMeters =
    0.75F;
// This is a last-resort rejoin after native pathfinding has already spent its full four-second budget.
// The previous 4.5 m ceiling could never catch a sprinting authoritative route: every timeout moved the proxy only one or two old waypoints while the route added another ~20 m.
// A recorded waypoint is a position the authoritative player actually occupied, so a bounded 40 m rejoin is safer than leaving the proxy permanently tens of metres behind.
inline constexpr float kRemoteNavigationSafeRecoveryMaximumDistanceMeters =
    40.0F;
// Position accuracy wins once animation-driven catch-up can no longer keep a replica near its authoritative marker.
// A short sustained threshold avoids reacting to one interpolation spike, while the emergency threshold bounds the time a stuck puppet can remain visibly tens of metres behind.
...
// A stale traversal played many metres after its source action looks worse than an explicit route recovery.
// The pre-takeoff edge and reliable TCP transaction now make a short validity window sufficient.
...
// Animation samples are an optional overlay on the authoritative transform stream.
// They must never outlive the transform interval they describe or be applied to a render point far away on the sender's monotonic timeline.
...
// FORCE_PED_MOTION_STATE on a taskless CREATE_PED proxy reports success but leaves its skeleton in a T-pose.
// A long, non-navmesh visual task keeps the native gait graph alive while network coordinates remain authoritative.
...
// Direction is expressed in the sender ped's local axes.
// RDR2's native heading uses 0 degrees at +Y and increases towards -X, which differs from the mathematical +X convention used by atan2(y, x).
// Keeping this explicit prevents forward motion from being misclassified as a 90-degree strafe.
...
// Converts world velocity to the heading convention consumed by RDR2's ENTITY/TASK natives.
// A nearly stationary sample retains fallbackHeading.
[[nodiscard]] float MovementHeadingFromVelocity(
    const Vec3& velocity,
    float fallbackHeading) noexcept;

// Projects world velocity into the ped's forward/right axes using RDR2's native heading convention (heading 0 points along +Y).
...
// Receive age bounds a stopped animation stream, while the sender-tick check prevents a current packet from being paired with a distant buffered render point.
// A zero sender tick means that only receive-age validation is available (used by old/synthetic loopback peers).
...
// The visual gait controller yields completely to mounts and protected native physics.
// Ordinary coordinates are never written by this decision; bounded hard recovery is evaluated separately after sustained divergence.
[[nodiscard]] bool ShouldRunAnimGraphVisualController(
    bool mounted,
    bool protectedPhysicalAnimation) noexcept;

// Chooses the visual gait advertised to RDR2's native task graph.
// A validated sender state wins; desiredMoveBlend is a fail-closed fallback for recordings or peers that only carry the transform stream.
...
// The visual task is deliberately long and straight: it exists only to keep the native gait graph evaluating.
// It never owns the replicated root and it never invokes navmesh/pathfinding.
...
// Traversal is a short physical hand-off inside the direct-root engine.
// The reliable action may arrive before the delayed render point reaches its takeoff position, so either the matching semantic action or proximity to the recorded anchor may commit it.
// Locomotion resumes only after RDR2 yields the jump/climb task.
[[nodiscard]] bool ShouldStartDirectReplicaTraversal(
    const DirectReplicaTraversalStartInput& input) noexcept;

// A ledge fall has no jump transaction.
// Launch local physics once on the transition into the authoritative airborne mode, then let native falling own the skeleton until it lands.
...
// RDR2's reported ped position is authoritative.
// Ground probing is used only to remove a small local height error (for example on a nearby slope); it must never pull a replica to another floor, a river bed, or distant terrain.
[[nodiscard]] Vec3 SelectGroundSafePosition(
    const Vec3& reportedPosition,
    std::optional<float> groundZ) noexcept;

// Produces a frame-rate-independent velocity for ordinary updates.
// A stale local replica receives a proportional physical velocity assist, while coordinate warping still requires an authoritative target discontinuity.
...
// Therefore a gait transition here is a confirmed animation-speed change and should replace the current movement task exactly once.
[[nodiscard]] bool ShouldRefreshRemoteLocomotionTask(
    const RemoteLocomotionTaskRefreshInput& input) noexcept;

// Separates root motion from weapon actions.
// A moving aim task may own the upper-body pose only while the replica is close to its network marker.
...
// Hysteresis for the optional physical tracking layer.
// It must not repeatedly toggle when interpolation noise hovers near one threshold.
[[nodiscard]] bool ShouldApplyRemotePhysicsAssist(
    bool currentlyActive,
    float positionErrorMeters) noexcept;

// A blocked sample means that the authoritative target keeps moving while the visible ped does not, or that a large error is no longer improving.
...
// Navmesh recovery has a wide enter/exit hysteresis.
// It is never allowed to fight ragdoll, climbing physics or a mounted replica.
...
// Once a peer is far beyond a small local detour, native navmesh should route directly to its current location instead of replaying historical footsteps.
[[nodiscard]] bool ShouldUseDirectRemoteNavigationTarget(
    float positionErrorMeters) noexcept;

// A moving route must replace its navmesh destination much sooner than the task timeout.
// Urgent refresh still has a small floor to prevent restarting the native task every rendered frame.
...
// Recovery is a short obstacle-detour tool.
// It must yield after a bounded period so native pathfinding cannot suppress all catch-up motion for tens of seconds while following obsolete route points.
[[nodiscard]] bool HasRemoteNavigationRecoveryTimedOut(
    std::uint64_t activeForMs,
    bool physicsInterrupted) noexcept;

// A coordinate correction is the last step of obstacle recovery, never the ordinary tracking mechanism.
// It may advance to the newest valid recorded route waypoint inside a hard distance bound only after pathfinding has exhausted its bounded time budget.
...
// Rejoin its current marker after a bounded grace period, but never interrupt ragdoll/climb physics, a mount relationship or an authoritative teleport already handled by the normal discontinuity path.
...
// Traversal is recorded at the authoritative route position.
// If the visual proxy is behind, defer jump/climb until it reaches that position instead of playing the action several metres too early.
...
// Selects a point ahead along the buffered route rather than extending the most recent velocity tangent.
// Curvature reduces the look-ahead so a sharp corner cannot aim the task through nearby geometry.
...
// One primary owner may issue a full-body task at a time.
// This pure selector makes ownership order explicit and testable.
...
// Buffers accepted snapshots on a receiver-local mapping of the sender's monotonic timeline.
// Arrival times are retained only for freshness and jitter estimation, so a delayed UDP datagram cannot distort route timing.
// Keeps a few old remote-player updates.
// We show them slightly late to make movement smoother.
// If updates stop, the player eventually stops moving.
